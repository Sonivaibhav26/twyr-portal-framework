 ...
 * Description	: The Twy'r Portal Application dependency manager and service/component loader
 *
 */
...
var twyrLoader = prime({
	'constructor': function(basePath, module) {
		...
		if(!module.load || !module.start || !module.stop || !module.unload){
			return;
		}
	...
	'start': function(callback) {
		...
		.then(function(status) {
			...
			return self._startServicesAsync();
		})
		.then(function(status) {
			if(!status) throw status;
	...
	'stop': function(callback) {
		...
		.then(function(status) {
			...
			finalStatus.push(status);
			return self._uninitializeComponentsAsync();
		})
		.then(function(status) {
			if(!status) throw status;
		...
		.then(function(status) {
			...
			finalStatus.push(status);
			return self._uninitializeServicesAsync();
		})
		.then(function(status) {
			if(!status) throw status;
	...
	'_loadUtilities': function(callback) {
		try {
			...
			for(var idx in definedUtilities) {
				...
				this.$module.$utilities[utility.name] = promises.promisify(utility.method.bind(this.$module));
			}
